...
class SpecReporter extends events.EventEmitter {
    ...
    printSuiteRealtime (suite) {
        return this.preferredIndent.repeat(this.suiteIndents[suite.cid][suite.uid]) + suite.title
    }

    printTestResultRealtime (test, state) {
        const initialIndentation = this.preferredIndent.repeat(this.suiteIndents[test.cid][test.parentUid] + 1)
        const testStatus = this.chalk[this.getColor(state)](this.getSymbol(state))
